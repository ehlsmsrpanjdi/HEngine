...
public:
	...
	void Rotate(float _x, float _y = 0, float _z = 0) {
		...
		XMVECTOR additionalRotationX = XMQuaternionRotationAxis(XMVectorSet(1, 0, 0, 0), _x); // X축 (Pitch)
		XMVECTOR additionalRotationY = XMQuaternionRotationAxis(XMVectorSet(0, 1, 0, 0), _y); // Y축 (Yaw)

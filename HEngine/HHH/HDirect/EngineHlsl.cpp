...
{
	HlslResourceMap.insert(std::make_pair("DEFAULT", std::make_shared<DefaultResource>()));
	//HlslResourceMap.insert(std::make_pair("SKY", std::make_shared<SkyResource>()));
...
void EngineHlsl::CreateHlsl(std::shared_ptr<GraphicDevice> _Device, std::string_view _str, std::shared_ptr<HS> _Hlsl)
{
	...
	if (HString::Upper(str) == "DEFAULT") {
		D3D11_INPUT_ELEMENT_DESC temp[] =
		{
			...
			{ "BONEINDICES",  0, DXGI_FORMAT_R32_UINT,           0, 24, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "BONEINDICES",  1, DXGI_FORMAT_R32_UINT,           0, 28, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "BONEINDICES",  2, DXGI_FORMAT_R32_UINT,           0, 32, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "BONEINDICES",  3, DXGI_FORMAT_R32_UINT,           0, 36, D3D11_INPUT_PER_VERTEX_DATA, 0 },

			{ "BONEWEIGHTS",  0, DXGI_FORMAT_R32_FLOAT,          0, 40, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "BONEWEIGHTS",  1, DXGI_FORMAT_R32_FLOAT,          0, 44, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "BONEWEIGHTS",  2, DXGI_FORMAT_R32_FLOAT,          0, 48, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "BONEWEIGHTS",  3, DXGI_FORMAT_R32_FLOAT,          0, 52, D3D11_INPUT_PER_VERTEX_DATA, 0 },
